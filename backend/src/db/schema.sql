...
-- Migration: 20260714_native_radio_agent_voting
-- Planned schedule timestamps stay separate from the actual transition times.
...
-- A pre-existing stale active round must not prevent the safety index from
-- being created. Legacy active rounds have no authoritative schedule and must
-- be cancelled; among scheduled rounds keep only the newest active round.
...
SET album_art_url = '/uploads/next-song-voting/fallback.png', album_art_sha256 = NULL
