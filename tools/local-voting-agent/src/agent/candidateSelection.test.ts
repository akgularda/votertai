...
describe('candidate selection', () => {
  ...
  it('excludes the current and recently played songs from candidates', () => {
    const candidates = selectRandomCandidates(songs, 2, () => 0, new Set(['song-1']));

    expect(candidates.map((candidate) => candidate.songId)).toEqual(['song-2', 'song-3']);
  });
});
