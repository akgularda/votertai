...
function countedCandidatesForResolution(round: VotingRound): VotingCandidate[] {
  return round.votes.length > 0 ? withVoteCounts(round).candidates : round.candidates;
}
...
function getResolution(round: VotingRound, rng: () => number): {
  ...
  const counted = countedCandidatesForResolution(round);

  if (counted.every((candidate) => candidate.votes === 0)) {
    return {
...
export function resolveRound(round: VotingRound, rng: () => number = Math.random, now: Date = new Date()): VotingRound {
  ...
  return {
    ...
    candidates: countedCandidatesForResolution(round),
